...
load_model = False
current_step = 0
folder_name="double-vanilla-discrete-A1"
path = "results/"
...
restoring_backup_name = "double-vanilla-dqn"
...
class Convolutional:
    ...
    def __init__(self):
        ...
        # Multiply Q-values by a OneHotEncoding to only take the chosen ones.
...
def print_debug_states(tf_session, QNet, raw_input):
...
with tf.Session() as sess:
    # Use a Double Vanilla Q-Network
    mainQN = Convolutional()
    targetQN = Convolutional()
    ...
    if load_model:
        print('Loading Model...')
        picklehandler = open(restoring_backup_folder + 'double-dqn-' + str(current_step) + ".pickle", 'rb')
        ...
        restoring_backup = path + folder_name + "/double-dqn-" + str(current_step) + ".meta"
    ...
    for i in range(current_step+1, num_episodes):
        ...
        while not d:
            ...
            if np.random.rand(1) < e or total_steps < pre_train_steps:
                ...
                prediction, qvalues = sess.run([mainQN.prediction, mainQN.Qout], feed_dict={mainQN.x: [s]})
        ...
        if i % 3000 == 0 and i != 0:
            saver.save(sess, restoring_backup_folder + 'double-dqn', global_step=i)
            with open(restoring_backup_folder + 'double-dqn-' + str(i) + ".pickle", 'wb') as file:
                dictionary = {
                    "epsilon": e,
    ...
    saver.save(sess, 'double-dqn', global_step=i)
# end
