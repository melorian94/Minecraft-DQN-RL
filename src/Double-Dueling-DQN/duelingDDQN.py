...
restoring_backup_name = "double-dueling-vanilla-dqn"
...
with tf.Session() as sess:
    # Use a Double Dueling Q-Network
    mainQN = Dueling()
    targetQN = Dueling()
    ...
    if load_model:
        print('Loading Model...')
        picklehandler = open(restoring_backup_folder + restoring_backup_name + '-' + str(current_step) + ".pickle", 'rb')
        ...
        restoring_backup = path + folder_name + "/" + restoring_backup_name + "-" + str(current_step) + ".meta"
    ...
    for i in range(num_episodes):
        ...
        while not d:
            ...
            if total_steps > pre_train_steps:
                if total_steps % update_freq == 0:
                    ...
                    if is_debug:
                        print_debug_states(sess, mainQN, trainBatch[0, 0])
        ...
        if i % 3000 == 0 and i != 0:
            saver.save(sess, restoring_backup_folder + restoring_backup_name, global_step=i)
            with open(restoring_backup_folder + restoring_backup_name + '-' + str(i) + ".pickle", 'wb') as file:
                dictionary = {
                    "epsilon": e,
    ...
    saver.save(sess, restoring_backup_name, global_step=i)
# end
