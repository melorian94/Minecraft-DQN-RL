...
restoring_backup_name = "double-stacked-dqn"
...
class stacked_experience_buffer:
    def __init__(self, buffer_size=50000):  # Stores episodes
        self.buffer = []
...
with tf.Session() as sess:
    # Use a Double Stacked Q-Network
    mainQN = StackedConvolutional(trace_length)
    targetQN = StackedConvolutional(trace_length)
    ...
    if load_model:
        print('Loading Model...')
        picklehandler = open(restoring_backup_folder + 'double-stacked-dqn-' + str(current_step) + ".pickle", 'rb')
        ...
        restoring_backup = path + folder_name + "/double-stacked-dqn-" + str(current_step) + ".meta"
    ...
    for i in range(num_episodes):
        ...
        if i % 3000 == 0 and i != 0:
            saver.save(sess, restoring_backup_folder + 'double-stacked-dqn', global_step=i)
            with open(restoring_backup_folder + 'double-stacked-dqn' + str(i) + ".pickle", 'wb') as file:
                dictionary = {
                    "epsilon": e,
    ...
    saver.save(sess, 'double-stacked-dqn', global_step=i)
# end
